...
const ListCerimo = ({onEdit, onDelete}) =>{
    ...
    const loadCerimo = async () => {
        try {
          const response = await axios.get('http://localhost:5000/api/rota/registroCeri');
  ...
  {cerimonialistas.map(cerimonialistas => (
    ...
    <span key={cerimonialistas.id} id='list'>
      ...
      <div id='ButtonsU'>
        ...
        <button onClick={() => onEdit(cerimonialistas)} className='btnU'>Editar</button>
        <button onClick={() => onDelete(cerimonialistas.id)} className='btnU'>Excluir</button>
      </div>
    </span>
